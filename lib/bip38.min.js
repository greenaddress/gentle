var bip38 = (function() {
    ...
    return {
        encrypt: function(input, salt_a) {
            if (!salt_a) {
                var salt_a = wordArrayToBytes(Bitcoin.CryptoJS.SHA256(Bitcoin.CryptoJS.SHA256(
                    bytesToWordArray(input.data)))).slice(0, 4);
            }
            ...
            var message = wordArrayToBytes(
                Bitcoin.CryptoJS.AES.encrypt(
                    bytesToWordArray(message),
                    bytesToWordArray(derivedhalf2),
        ...
        decrypt: function(input) {
            ...
            var encodedkey = scrypt.encode_utf8(input.key);
            var derivedBytes = scrypt.crypto_scrypt(encodedkey,
                                           salt, 16384, 8, 8, 64);

            var encrypted = bytesToWordArray(bytes.slice(0, -4));
            var decrypted = wordArrayToBytes(
                Bitcoin.CryptoJS.AES.decrypt(
                    Bitcoin.CryptoJS.lib.CipherParams.create({ciphertext: encrypted}),
                    bytesToWordArray(derivedBytes.subarray(32, 32+32)),
                    {mode: Bitcoin.CryptoJS.mode.ECB,
                     padding: Bitcoin.CryptoJS.pad.NoPadding}));

            for (var x = 0; x < 32; x++) decrypted[x] ^= derivedBytes[x];

            var hash = wordArrayToBytes(Bitcoin.CryptoJS.SHA256(Bitcoin.CryptoJS.SHA256(
                bytesToWordArray(decrypted))));
            for (var i = 0; i < 4; i++) {
                if (hash[i] != salt[i]) {
